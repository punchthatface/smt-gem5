...
#include <math.h>
#include <iostream>
#include <stdint.h>   // PHASE2 PATCH: needed for uint64_t used by gem5 work markers.
#include <string.h>   // PHASE2 PATCH: makes strcmp declaration explicit.

// PHASE2 PATCH:
// gem5 m5ops support. When compiled with -DUSE_M5OPS and linked with libm5.a,
// m5_work_begin() emits a gem5 pseudo-instruction. Our gem5 runner catches that
// event and performs L1D writeback+invalidate on the simulator side.
// When not compiled with USE_M5OPS, this becomes a no-op so native PARSEC runs
// still work normally.
#ifdef USE_M5OPS
...
// PHASE2 PATCH:
// Controls how often swaptions emits a gem5 work marker.
// -fi 0 disables markers.
// -fi N emits one marker before every Nth swaption computation.
// The marker does not flush anything by itself; gem5 performs L1D cleanup.
int phase2_flush_interval = 0;
...
// PHASE2 PATCH:
// Emit a gem5-visible domain/cleanup marker at coarse outer-loop boundaries.
// This is intentionally placed outside the HJM numerical kernel so the benchmark
// result is not changed. It only gives gem5 a point where our runner can perform
// L1D writeback+invalidate for the flush-assisted policy.
static inline void phase2_maybe_work_marker(int idx)
{
  if (phase2_flush_interval > 0 && idx >= 0 &&
      (idx % phase2_flush_interval) == 0) {
    fprintf(stderr, "PHASE2_MARKER_REACHED idx=%d\n", idx);
    fflush(stderr);
    m5_work_begin((uint64_t)idx, 0);
  }
}
...
struct Worker {
  ...
  void operator()(const tbb::blocked_range<int> &range) const {
    ...
    for(int i=begin; i!=end; i++) {
      // PHASE2 PATCH:
      // Marker before one unit of swaption work. In gem5, this can trigger
      // simulator-side cache cleanup when the flush policy is enabled.
      phase2_maybe_work_marker(i);
...
void * worker(void *arg){
  ...
  for(int i=beg; i < end; i++) {
     // PHASE2 PATCH:
     // Marker before one unit of swaption work. This is the pthread path used by
     // gcc-pthreads. For our Phase 2 gem5 runs we usually use -nt 1, so this is
...
void print_usage(char *name) {
  ...
  // PHASE2 PATCH:
  // New option used by gem5 Phase 2. It controls how often this benchmark emits
  // m5_work_begin markers. The simulator-side runner decides whether those
  // markers cause L1D writeback+invalidate.
  fprintf(stderr,"\t-fi [gem5 work marker interval; 0 disables]\n");
}
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
	...
	__parsec_bench_begin(__parsec_swaptions);
	  ...
	  // PHASE2 PATCH:
	  // Accept -fi N. This does not change the benchmark's computation.
	  // It only controls gem5 work markers for simulator-side flushing.
	  else if (!strcmp("-fi", argv[j])) {phase2_flush_interval = atoi(argv[++j]);}
          else {
        ...
        // PHASE2 PATCH:
        // Print marker configuration so logs clearly show whether a Phase 2
        // marker-enabled swaptions binary is being used.
        printf("Phase2 gem5 work marker interval: %d\n", phase2_flush_interval);
        // PHASE2 PATCH:
        // Startup marker to confirm swaptions m5ops are recognized by gem5
        // before entering the pthread worker path.
        if (phase2_flush_interval > 0) {
            fprintf(stderr, "PHASE2_STARTUP_MARKER\n");
            fflush(stderr);
            m5_work_begin(999999, 0);
        }
    ...
    // PHASE2 PATCH:
    // In gem5 SE mode, pthread behavior can be noisy for this benchmark.
    // For our Phase 2 runs we use -nt 1, so directly call the worker function.
    // This preserves the same computation while making gem5 work markers reliable.
    if (nThreads == 1) {
        threadIDs[0] = 0;
        worker(&threadIDs[0]);
    } else {
